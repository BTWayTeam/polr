...
class LinkHelper {
	...
	static public function checkIfDomainAllowed($long_link)
	{
		/**
		 * Provided a long link (string),
		 * detect whether the link belongs to an URL shortener.
		...
		$link_info = parse_url($long_link);
		$url_segment = $link_info['host'];

		foreach ($allowed_domains as $allowed_domain) {
			if (strstr($url_segment, $allowed_domain) !== false) {
				return true;
			}
		}
		return false;
